...
from autoregressive.models.far_triple import FAR_models
...
def main(args):
    ...
    if rank == 0:
        ...
        wandb.init(
            project="LlamaGen-C2I",        # set the wandb project where this run will be logged
            name=args.exp_name,         # set the exp name
    ...
    model = FAR_models[args.gpt_model](
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for x1, x2, x3, y in loader:
            ...
            z_indices_1 = x1.reshape(x1.shape[0], -1)
            z_indices_2 = x2.reshape(x2.shape[0], -1)
            z_indices_3 = x3.reshape(x3.shape[0], -1)
            c_indices = y.reshape(-1)
            assert z_indices_1.shape[0] == z_indices_2.shape[0] == z_indices_3.shape[0] == c_indices.shape[0]
            with torch.cuda.amp.autocast(dtype=ptdtype):
                _, _, _, loss = model(cond_idx=c_indices,
                                      idx_1=z_indices_1, idx_2=z_indices_2, idx_3=z_indices_3,
                                      targets_1=z_indices_1, targets_2=z_indices_2, targets_3=z_indices_3,
                                      )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gpt-model", type=str, choices=list(FAR_models.keys()), default="GPT-B")
