...
def main(args):
    ...
    for x, y in loader:
        ...
        y = y.to(device)
        with torch.no_grad():
            _, _, _, [_, _, indices_vis], [_, _, indices_sem_mid], [_, _, indices_sem_high] = vq_model.encode(x_all)
        codes_vis = indices_vis.reshape(x.shape[0], num_aug, -1)
        codes_sem_mid = indices_sem_mid.reshape(x.shape[0], num_aug, -1)
        codes_sem_high = indices_sem_high.reshape(x.shape[0], num_aug, -1)

        x_vis = codes_vis.detach().cpu().numpy()    # (1, num_aug, args.image_size//16 * args.image_size//16)
        x_sem_mid = codes_sem_mid.detach().cpu().numpy()
        x_sem_high = codes_sem_high.detach().cpu().numpy()
        train_steps = rank + total
        np.save(f'{args.code_path}/{args.dataset}{args.image_size}_codes/{train_steps}_vis.npy', x_vis)
        np.save(f'{args.code_path}/{args.dataset}{args.image_size}_codes/{train_steps}_sem_mid.npy', x_sem_mid)
        np.save(f'{args.code_path}/{args.dataset}{args.image_size}_codes/{train_steps}_sem_high.npy', x_sem_high)
