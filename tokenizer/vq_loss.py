...
from tokenizer.models.lpips import LPIPS
from tokenizer.models.discriminator_patchgan import NLayerDiscriminator as PatchGANDiscriminator
from tokenizer.models.discriminator_stylegan import Discriminator as StyleGANDiscriminator
from tokenizer.models.clip_encoder import CLIPVisionTower
from tokenizer.models.dino_encoder import DinoVisionTower
...
class VQLoss_dual_codebook(nn.Module):
    def __init__(self, disc_start, disc_loss="hinge", disc_dim=64, disc_type='patchgan', image_size=256,
                 disc_num_layers=3, disc_in_channels=3, disc_weight=1.0, disc_adaptive_weight=False,
                 gen_adv_loss='hinge', reconstruction_loss='l2', reconstruction_weight=1.0,
                 codebook_weight=1.0, perceptual_weight=1.0,
        ...
        if with_clip_supervision:
            self.clip_model = CLIPVisionTower("openai/clip-vit-large-patch14").eval()
    ...
    def forward(self,
                codebook_loss_sem, codebook_loss_vis,
                inputs, reconstructions,
                disentangle_loss, semantic_feat,
                optimizer_idx, global_step, last_layer=None,
                logger=None, log_every=100):
        # generator update
        if optimizer_idx == 0:
            ...
            if semantic_feat is not None:
                semantic_loss = self.clip_model(inputs.contiguous(), semantic_feat)
                semantic_loss = torch.mean(semantic_loss)
            else:
                assert self.with_clip_supervision == False
                semantic_loss = torch.mean(torch.zeros_like(rec_loss))
            ...
            loss = self.rec_weight * rec_loss + \
                   self.perceptual_weight * p_loss + \
                   disc_adaptive_weight * disc_weight * generator_adv_loss + \
                   codebook_loss_sem[0] + codebook_loss_sem[1] + codebook_loss_sem[2] + \
                   codebook_loss_vis[0] + codebook_loss_vis[1] + codebook_loss_vis[2] + \
                   self.semantic_weight * semantic_loss + disentangle_loss
            ...
            if global_step % log_every == 0:
                ...
                generator_adv_loss = disc_adaptive_weight * disc_weight * generator_adv_loss
                logger.info(f"(Generator) rec_loss: {rec_loss:.4f}, perceptual_loss: {p_loss:.4f}, "
                            f"vq_loss_sem: {codebook_loss_sem[0]:.4f}, commit_loss_sem: {codebook_loss_sem[1]:.4f}, entropy_loss_sem: {codebook_loss_sem[2]:.4f}, "
                            f"codebook_usage_sem: {codebook_loss_sem[3]:.4f}, "
                            f"vq_loss_vis: {codebook_loss_vis[0]:.4f}, commit_loss_vis: {codebook_loss_vis[1]:.4f}, entropy_loss_vis: {codebook_loss_vis[2]:.4f}, "
                            ...
                            f"disc_adaptive_weight: {disc_adaptive_weight:.4f}, disc_weight: {disc_weight:.4f}, semantic_loss: {semantic_loss:.4f}")
                if dist.get_rank() == 0:
                    wandb.log({
                        "rec_loss": rec_loss,
                        "perceptual_loss": p_loss,
                        "disentangle_loss": disentangle_loss,
                        "codebook_loss_sem": codebook_loss_sem[0],
                        "commit_loss_sem": codebook_loss_sem[1],
                        "entropy_loss_sem": codebook_loss_sem[2],
                        "codebook_usage_sem": codebook_loss_sem[3],
...
class VQLoss_triple_codebook(nn.Module):
    def __init__(self, disc_start, disc_loss="hinge", disc_dim=64, disc_type='patchgan', image_size=256,
        ...
        self.with_clip_supervision = with_clip_supervision
        if with_clip_supervision:
            self.clip_model = CLIPVisionTower("openai/clip-vit-base-patch16").eval()
            self.dinov2_model = DinoVisionTower("facebook/dinov2-small").eval()
            self.clip_model.requires_grad_(False)
    ...
    def forward(self,
        ...
        if optimizer_idx == 0:
            ...
            if semantic_feat_mid is not None:
                ...
                semantic_loss_mid = self.dinov2_model(inputs.contiguous(), semantic_feat_mid)
