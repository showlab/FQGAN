...
from tokenizer.vq_model_dual import VQ_models
from tokenizer.vq_loss import VQLoss_dual_codebook
...
def main(args):
    ...
    if rank == 0:
        ...
        experiment_dir = f"{args.results_dir}/{model_string_name}-{args.exp_name}"  # Create an experiment folder
        ...
        wandb.init(
            project="Tokenizer",    # set the wandb project where this run will be logged
            name=args.exp_name,
            config=args     # track hyperparameters and run metadata
        )
    else:
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for x, y in loader:
            ...
            with torch.cuda.amp.autocast(dtype=ptdtype):  
                recons_imgs, codebook_loss_sem, codebook_loss_vis, disentangle_loss, semantic_feat = vq_model(imgs)
                loss_gen = vq_loss(codebook_loss_sem, codebook_loss_vis,
                                   imgs, recons_imgs,
            ...
            if rank == 0 and (train_steps+1) % (args.log_every * 5) == 0:
                input_img = [wandb.Image(img.permute(1, 2, 0).detach().cpu().float().numpy()) for img in imgs[:4]]
                recon_img = [wandb.Image(img.permute(1, 2, 0).detach().cpu().float().numpy()) for img in recons_imgs[:4]]
                wandb.log({"input_image": input_img})
                wandb.log({"recon_img": recon_img})
            ...
            with torch.cuda.amp.autocast(dtype=ptdtype):
                loss_disc = vq_loss(codebook_loss_sem, codebook_loss_vis,
                                    imgs, recons_imgs,
                                    disentangle_loss, semantic_feat,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--with_clip_supervision", action='store_true')
    parser.add_argument("--with_disentanglement", action='store_true')
    parser.add_argument("--semantic-weight", type=float, default=0.5, help="semantic loss weight of CLIP")
    parser.add_argument("--disentanglement-ratio", type=float, default=0.1, help="disentanglement ratio")
