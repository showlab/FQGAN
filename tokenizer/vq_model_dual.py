...
class FeatPredHead(nn.Module):
    def __init__(self, input_dim=256, down_factor=16):
        ...
        self.grid_size = 256 // down_factor  # image size // down-sample ration
        self.width = 1024  # subject to CLIP/DINO model
        self.num_layers = 3
        self.num_heads = 8
...
class VQModel(nn.Module):
    def __init__(self, config: ModelArgs):
        ...
        # Quantizer for semantic head
        self.quantize_sem = VectorQuantizer(config.codebook_size, config.codebook_embed_dim,
                                            config.commit_loss_beta, config.entropy_loss_ratio,
                                            config.codebook_l2_norm, config.codebook_show_usage)
        self.quant_conv_sem = nn.Conv2d(config.z_channels, config.codebook_embed_dim, 1)
        ...
        print("Semantic codebook: [{} x {}]".format(config.codebook_size, config.codebook_embed_dim))

        # Pixel decoder
        input_dim = config.codebook_embed_dim * 2
        ...
        if self.config.with_clip_supervision:
            ...
            self.feat_pred_head = FeatPredHead(input_dim=config.codebook_embed_dim, down_factor=down_factor)
        else:
    ...
    def forward(self, input):
        h_sem, h_vis = self.encoder(input)
        h_sem = self.quant_conv_sem(h_sem)
        h_vis = self.quant_conv_vis(h_vis)

        quant_sem, emb_loss_sem, _ = self.quantize_sem(h_sem)
        quant_vis, emb_loss_vis, _ = self.quantize_vis(h_vis)

        if self.config.with_clip_supervision:
            semantic_feat = self.feat_pred_head(quant_sem)
        else:
            semantic_feat = None

        if self.config.with_disentanglement:
            disentangle_loss = self.compute_disentangle_loss(quant_vis, quant_sem)
        else:
            disentangle_loss = 0

        quant = torch.cat([quant_vis, quant_sem], dim=1)
        dec = self.decode(quant)

        return dec, emb_loss_sem, emb_loss_vis, disentangle_loss, semantic_feat

    def encode(self, x):
        h_sem, h_vis = self.encoder(x)
        h_sem = self.quant_conv_sem(h_sem)
        h_vis = self.quant_conv_vis(h_vis)

        quant_sem, emb_loss_sem, info_sem = self.quantize_sem(h_sem)
        quant_vis, emb_loss_vis, info_vis = self.quantize_vis(h_vis)

        return quant_sem, quant_vis, info_sem, info_vis

    def decode_code(self, indices_sem, indices_vis, shape_sem=None, shape_vis=None, channel_first=True):
        quant = self.quantize_vis.get_codebook_entry(indices_vis, shape_vis, channel_first)
        quant_sem = self.quantize_sem.get_codebook_entry(indices_sem, shape_sem, channel_first)

        quant = torch.cat([quant, quant_sem], dim=1)
        dec = self.decode(quant)
...
class Encoder(nn.Module):
    def __init__(self, in_channels=3, ch=128, ch_mult=(1,1,2,2,4), num_res_blocks=2, 
        ...
        self.semantic_head.append(FactorizedAdapter(down_factor))
        ...
        # end
        self.norm_out_sem = Normalize(block_in, norm_type)
        self.conv_out_sem = nn.Conv2d(block_in, z_channels, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        # semantic head
        for blk in self.semantic_head:
            h_sem = blk(h_sem)

        h_sem = self.norm_out_sem(h_sem)
        h_sem = nonlinearity(h_sem)
        h_sem = self.conv_out_sem(h_sem)
